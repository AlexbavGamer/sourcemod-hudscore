...
#define UPDATE_URL "http://mixbrasil.000webhostapp.com/updatefile.txt"
...
public void OnPluginStart()
{
	...
	LoadTranslations("hudscoreboard.phrases");
	CreateConfig();
	...
	for (new i = 1; i < MaxClients; i++) {
		if(IsClientValid(i)) {
			if(Scores[i] == null) {
				...
				score.InitiateTimer(ShowHudScore);
				Scores[i] = score;
			}
		}
	}
...
public Action Command_EnableHud(int client, int args)
{
	...
	global_option_hud = !global_option_hud;
	...
	if(action == CookieMenuAction_DisplayOption) 
	{
		...
		if(global_option_hud) {
			...
			if(option_show_hud[client]) {
				...
				Scores[client].InitiateTimer(ShowHudScore);
			}
			else 
			{
				SetClientCookie(client, cookie_show_hud, "Off");
				Scores[client].StopTimer();
			}
		} else {
...
public void CreateConfig() {
	...
	g_hud_enabled = CreateConVar("g_hud_enabled", "True", "Display Hud Scoreboard if enabled");
...
public Action ShowHudScore(Handle timer, int clientId) 
{
	...
	{	
		...
		SetHudTextParams(-1.0, 0.1, 2.0, 255, 255, 255, 255, 1, 1.0, 1.0, 1.0);
